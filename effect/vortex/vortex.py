...
import os
from scipy.ndimage import map_coordinates  # For polar remap
...
def polar_swirl(region, center, angle, image, strength=1.0):
    """
    Classical swirl: Remap pixels radially around center by angle.
    Fixed: Per-channel map_coordinates for RGBA.
    """
    if len(region) == 0:
        return image
    ...
    # Swirl: Add angle * (r / max_r) for decay from center
    max_r = np.max(r)
    swirl_theta = theta + angle * (r / max_r) * strength

    # New positions
    new_y = r * np.sin(swirl_theta) + center[0]
    new_x = r * np.cos(swirl_theta) + center[1]

    # Fixed: Remap per channel (spatial only, replicate for each)
    remapped = np.zeros_like(image)
    for ch in range(c):
        coords = np.stack([new_y, new_x])  # 2D (2, H, W)
        remapped[:, :, ch] = map_coordinates(image[:, :, ch], coords, order=1, mode='constant')

    # Patch only to region (localized effect)
...
def run(params):
    ...
    for i, (lines, angle) in enumerate(zip(split_paths, angles)):
        ...
        image = polar_swirl(region, center, angle, image, strength)
